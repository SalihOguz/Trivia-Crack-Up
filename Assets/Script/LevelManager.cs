...
public class LevelManager : MonoBehaviour {
	...
	public void CheckLevelUp()
	{
		if (gameManager.player1.score >= av.levelUpgradeScores[gameManager.player1.level + 1]) // level up
		{
			...
			levelUpScreen.transform.Find("CharacterNameText").GetComponent<Text>().text = I2.Loc.ScriptLocalization.Get(av.avatarNames[Mathf.Clamp(gameManager.player1.avatarId + 1, 1, av.avatarSprites.Length - 1)]);
